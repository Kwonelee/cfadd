...
OUTPUT_FILE: Path = Path('cf-add.txt')
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if geoip2_database is None:
                raise RuntimeError(
                    'geoip2 not installed; run: pip install -r scripts/requirements.txt'
                )
...
def _get_reader() -> Reader:
    ...
    global _reader
    if geoip2_database is None:
        raise RuntimeError('geoip2 not installed; run: pip install -r scripts/requirements.txt')
    if _reader is None:
        _ensure_mmdb()
...
def main() -> int:
    ...
    try:
        ...
        with tmp.open('w', encoding='utf-8') as f:
            f.write(f'#{len(entries)} cfadd updated at {timestamp}\n')
            for ip, port in sort_endpoints(all_ips):
                ip_port = f'{ip}:{port}'
                f.write(f'{ip_port}#{entries[ip_port]}\n')
